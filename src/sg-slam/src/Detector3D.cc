...
// 6. compute every point cloud cluster parameter,and find best object cloud cluster
    ...
    for(auto vit = vcloud_cluster.begin();vit != vcloud_cluster.end(); ++vit)
    {
        ...
        float similar = GetSimilarity(rect2d,rect3d,points_num);
...
float Detector3D::GetSimilarity(const cv::Rect_<float> & r1, const cv::Rect_<float> & r2, size_t points_num)
{
  ...
  // the more point clouds of cluster there are, the higher the similarity
  return (overlap * points_num_score) / deviate;
}
